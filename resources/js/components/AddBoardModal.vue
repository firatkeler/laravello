<template>
    <Modal :width="400" :height="250" :show="show" @closed="$emit('closed')">
        <div class="flex">
            <div :class="[colors[color]]" class="rounded-sm p-4 text-black w-full mr-2">
            ...
            <div>
                <div class="flex justify-between mb-2" v-for="(row, index) in colorGrid" :key="index">
        ...
        <div class="mt-4">
            <button @click="addBoard" :disabled="cannotAdd" :class="[colors[color]]" class="rounded-sm py-2 px-4 text-white hover:opacity-75 cursor-pointer disabled:opacity-75">Add</button>
        </div>
    </Modal>
</template>
